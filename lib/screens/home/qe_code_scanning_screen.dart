...
class _QrScanScreenState extends State<QrScanScreen>
  ...
  void initState() {
    ...
    // ✅ FIXED: Connection listener with camera restart
    _connectivitySub = Connectivity().onConnectivityChanged.listen((result) async {
      ...
      if (result == ConnectivityResult.none) {
        ...
        // 🟢 Internet on - Green line dikhao + CAMERA RESTART
        if (mounted && _isScreenActive) {
          setState(() {
            _isOnline = true;
            _wasOffline = false;
          ...
          _showConnectionStatusBar("Connection restored", Colors.green);
          
          // ✅ AUTO RESTART CAMERA when internet returns
    ...
    // ✅ ADD: Camera state monitor
    _startCameraStateMonitor();
  }

  // ✅ NEW: Camera restart method
  Future<void> _restartCamera() async {
    try {
      if (!mounted || !_isScreenActive || _isProcessingLogout) return;
      
      // Check if camera is not already running
  ...
  // ✅ NEW: Camera state monitoring
  void _startCameraStateMonitor() {
    Timer.periodic(Duration(seconds: 3), (timer) {
      if (!_isScreenActive || _isProcessingLogout) {
        timer.cancel();
        return;
      }
      
      // Agar online hai aur camera nahi chal raha, to restart karein
      if (mounted && _isOnline && !controller.value.isRunning && !showErrorOverlay) {
        print("Camera monitor: Camera not running, restarting...");
        _restartCamera();
      }
    });
  }
  ...
  // ✅ MODIFIED: _handleTokenExpired without camera stop
  Future<void> _handleTokenExpired() async {
    if (_isProcessingLogout || _isHandlingConnection || !_isScreenActive)
      return;
    ...
    if (mounted &&
        ...
        !_isHandlingConnection &&
        _isScreenActive) {
      // ❌ REMOVED: Camera stop - sirf popup show karein
      setState(() {
        showErrorOverlay = true;
        errorTitle = "Session Expired";
        errorSubtitle = "Your token has expired. Please login again.";
  ...
  // ✅ MODIFIED: _showErrorDialog without camera stop
  void _showErrorDialog(String message, {bool showLoginButton = false}) {
    if (_isProcessingLogout || _isHandlingConnection || !_isScreenActive)
      return;
      ...
      body: SafeArea(
        child: Stack(
          children: [
            ...
            // Error Overlay (for token expired etc.)
            if (showErrorOverlay &&
                !_isProcessingLogout &&
                _isScreenActive) ...[
              Positioned.fill(
                          ...
                          onPressed: () async {
                            if (mounted &&
                              ...
                              if (errorSubtitle.contains("expired")) {
                                ...
                                // ✅ Ensure camera is running after popup close
                                await _restartCamera();
                              }
                            }
                          },
